"""D-PSGD ring gossip experiment with 5 hospitals.

Runs D-PSGD simulation for each seed and local_epochs setting.
Results saved to results/metrics/gossip/.
"""
...
def run_single(cfg: dict, seed: int, local_epochs: int) -> dict:
    """Run a single D-PSGD experiment."""
    ...
    logger.info(
        "D-PSGD seed=%d E=%d: final=%s comm_cost=%d",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run D-PSGD gossip experiments")
    parser.add_argument(
        "--config", type=Path,
    ...
    out_dir = PROJECT_ROOT / cfg["experiment"]["results_dir"] / "gossip"
    ...
    logger.info("Saved D-PSGD results to %s", out_path)
